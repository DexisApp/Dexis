...
export async function createApp() {
  ...
  const app = await NestFactory.create<NestExpressApplication>(AppModule, {
    ...
    rawBody: true,
    bodyParser: true,
    logger: Dexis.Dexis.stable ? ['log'] : ['verbose'],
  });
  ...
  if (Dexis.isSelfhosted && Dexis.telemetry.enabled) {
    const mixpanel = await import('mixpanel');
    mixpanel.init(Dexis.telemetry.token).track('selfhost-server-started', {
      version: Dexis.version,
    });
  }
