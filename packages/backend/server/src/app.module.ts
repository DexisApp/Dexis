...
export class AppModuleBuilder {
  private readonly modules: DexisModule[] = [];
  constructor(private readonly config: Config) {}

  use(...modules: DexisModule[]): this {
    modules.forEach(m => {
      ...
      if (requirements?.length) {
        ...
        if (nonMetRequirements.length) {
          ...
          new Logger(name).warn(
            ...
            ...nonMetRequirements.map(config => `  Dexis.${config}`)
          );
          return;
        }
      }
...
function buildAppModule() {
  const factor = new AppModuleBuilder(Dexis);
  ...
  // plugin modules
  Dexis.plugins.enabled.forEach(name => {
