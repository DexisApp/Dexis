# Building Dexis Desktop Client App
...
Before you start building Dexis Desktop Client Application, please following the same steps in [BUILDING#Prerequisites](./BUILDING.md#prerequisites) to install Node.js and Rust.
...
```shell
BUILD_TYPE=canary SKIP_NX_CACHE=1 yarn workspace @Dexis/electron generate-assets
```
...
```shell
BUILD_TYPE=canary SKIP_WEB_BUILD=1 HOIST_NODE_MODULES=1 yarn workspace @Dexis/electron make
```
...
yarn workspace @Dexis/electron package
yarn workspace @Dexis/electron make-squirrel
yarn workspace @Dexis/electron make-nsis
```
...
Finished 2 bundles at:
  › Artifacts available at: <Dexis-repo>/packages/frontend/electron/out/make
```
