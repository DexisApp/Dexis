...
Dexis client has both **Node.js** & **Rust** toolchains.
...
```sh
git clone https://github.com/dexisapp/Dexis
```
...
Note: use `strip` from system instead of `binutils` if you are running MacOS. [see problem here](https://github.com/dexisapp/Dexis/discussions/2840)
...
Also make sure you have built the `@Dexis/core` workspace before running E2E tests.
...
```shell
# there are `Dexis-local`, `Dexis-migration`, `Dexis-local`, `Dexis-prototype` e2e tests,
#   which are run under different situations.
cd tests/Dexis-local
yarn e2e
```
